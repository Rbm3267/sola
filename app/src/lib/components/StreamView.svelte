...
<div class="bg-white/95 backdrop-blur-2xl border border-slate-200/80 rounded-3xl p-6 shadow-[0_8px_30px_-6px_rgba(0,0,0,0.04)] w-full">
  <!-- Header -->
  <div class="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
    <div class="flex items-center gap-2">
      <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
      <span class="text-xs font-mono font-bold text-slate-800 uppercase tracking-wider">{config.title || 'Live Stream'}</span>
    </div>
    <span class="text-[10px] font-mono bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full border border-slate-200">
      0 ms Latency
    </span>
  </div>
  ...
  <div class="flex flex-col gap-2">
    {#each events as ev}
      <div class="flex items-center justify-between p-2.5 rounded-2xl bg-slate-50 border border-slate-100 text-xs font-mono">
        <div class="flex items-center gap-2">
          <span class="w-1.5 h-1.5 rounded-full {ev.type === 'success' ? 'bg-emerald-500' : ev.type === 'warning' ? 'bg-amber-500' : 'bg-sky-500'}"></span>
          <span class="text-slate-800 font-medium">{ev.message}</span>
        </div>

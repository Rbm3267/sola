  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
    ...
    <div class="flex items-center gap-8">
      ...
      <nav class="hidden md:flex items-center gap-1.5">
        <a 
          href="/" 
          class="px-4 py-2 rounded-xl text-xs font-semibold transition-all {currentPath === '/' ? 'text-amber-950 bg-amber-500/10 font-bold border border-amber-500/25 shadow-xs' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 border border-transparent'}">
          Overview
        </a>
        <a 
          href="/components" 
          class="px-4 py-2 rounded-xl text-xs font-semibold transition-all {currentPath.startsWith('/components') ? 'text-amber-950 bg-amber-500/10 font-bold border border-amber-500/25 shadow-xs' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 border border-transparent'}">
          Component Library
        </a>
        <a 
          href="/docs" 
          class="px-4 py-2 rounded-xl text-xs font-semibold transition-all {currentPath.startsWith('/docs') ? 'text-amber-950 bg-amber-500/10 font-bold border border-amber-500/25 shadow-xs' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 border border-transparent'}">
          Interactive Docs
        </a>
        <a 
          href="/demo" 
          class="px-4 py-2 rounded-xl text-xs font-semibold transition-all {currentPath.startsWith('/demo') ? 'text-amber-950 bg-amber-500/10 font-bold border border-amber-500/25 shadow-xs' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 border border-transparent'}">
          Playground
        </a>
      </nav>
    </div>

...
<div class="relative bg-white/95 backdrop-blur-2xl border border-slate-200/90 rounded-3xl p-6 sm:p-7 shadow-sm hover:shadow-md transition-all">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 mb-5">
    ...
    <div class="flex items-center gap-2 text-xs font-mono text-slate-600">
      <span class="px-2.5 py-1 rounded-xl bg-slate-100 border border-slate-200">
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-left font-mono text-xs">
      <thead>
        <tr class="border-b border-slate-100 text-[11px] text-slate-400 uppercase">
      ...
      <tbody class="divide-y divide-slate-100">
        {#each columns as col}
          <tr class="hover:bg-slate-50/80 transition-colors">
            <td class="py-2.5 font-bold text-slate-900 flex items-center gap-1.5">
              {#if col.isPrimary}
            ...
            <td class="py-2.5 text-center">
              {#if col.isPrimary}
                <span class="px-1.5 py-0.5 rounded text-[10px] bg-amber-50 text-amber-900 border border-amber-200 font-bold">PK</span>
              {:else if !col.isNullable}
                <span class="px-1.5 py-0.5 rounded text-[10px] bg-slate-100 text-slate-600 font-bold">NOT NULL</span>
              {:else}
            ...
            <td class="py-2.5 text-right text-slate-500">
              {#if col.foreignKey}
                <span class="inline-flex items-center gap-1 text-sky-600 hover:text-sky-700 cursor-pointer">
  ...
  <div class="flex items-center justify-between text-[11px] font-mono text-slate-400 pt-4 mt-4 border-t border-slate-100">

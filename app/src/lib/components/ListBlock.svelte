<script lang="ts">
  ...
  function getStatusBadge(status?: string) {
    ...
    if (lower.includes('active') || lower.includes('healthy') || lower.includes('online')) {
      return { bg: 'bg-emerald-500/10 text-emerald-700 border-emerald-500/20', dot: 'bg-emerald-500' };
    }
    if (lower.includes('degraded') || lower.includes('warn') || lower.includes('maint')) {
      return { bg: 'bg-amber-500/10 text-amber-700 border-amber-500/20', dot: 'bg-amber-500' };
    }
    return { bg: 'bg-rose-500/10 text-rose-700 border-rose-500/20', dot: 'bg-rose-500' };
  }
</script>
...
<div class="group relative bg-white/90 backdrop-blur-xl border border-slate-200/80 rounded-2xl p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_-6px_rgba(14,165,233,0.12)] hover:border-sky-300/80 transition-all duration-200 overflow-hidden w-full">
  ...
  <!-- Header -->
  <div class="flex items-center justify-between mb-4 pb-3 border-b border-slate-100 relative z-10">
  ...
  <div class="flex flex-col divide-y divide-slate-100/80 relative z-10">
    {#each config.items as item}
      ...
      <div class="py-3 first:pt-0 last:pb-0 flex items-center justify-between gap-4 group/row hover:bg-slate-50/60 px-2 -mx-2 rounded-xl transition-colors">
        <div class="flex items-center gap-3 min-w-0">
          <div class="w-7 h-7 rounded-lg bg-slate-100 border border-slate-200/60 flex items-center justify-center text-slate-500 shrink-0">
          ...
          <div class="min-w-0">
            <div class="text-xs font-bold text-slate-900 truncate font-mono">{item.label}</div>

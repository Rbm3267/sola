...
<div class="relative bg-white/95 backdrop-blur-2xl border border-rose-200/90 rounded-3xl p-6 sm:p-7 shadow-sm hover:shadow-md transition-all">
  
  <!-- Outer Glow Accent -->
  <div class="absolute top-0 right-0 w-32 h-32 bg-rose-500/5 rounded-full blur-3xl pointer-events-none"></div>

  <!-- Header -->
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 mb-5">
    <div>
      <div class="flex items-center gap-2 mb-1.5">
        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-rose-50 border border-rose-200 text-rose-900 font-mono font-black text-xs">
          <span class="w-2 h-2 rounded-full bg-rose-600 animate-ping"></span>
          {config.severity || 'P1 - Critical'}
        </span>
        <span class="text-xs font-mono font-bold text-slate-500">{config.incidentId || 'INC009481'}</span>
      </div>
    ...
    <div class="flex items-center gap-3 bg-slate-50 p-2.5 rounded-2xl border border-slate-200">
      <div class="text-right">
        <span class="text-[10px] font-mono text-slate-400 block uppercase">SLA Target Breach</span>
        <span class="text-sm font-mono font-black text-rose-600">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5 text-xs font-mono">
    <div class="p-3 rounded-2xl bg-slate-50/80 border border-slate-100">
      ...
      <span class="font-bold text-slate-800">{config.blastRadius || '42,000 Active Sessions • Checkout Ingress'}</span>
    </div>
    <div class="p-3 rounded-2xl bg-slate-50/80 border border-slate-100">
      <span class="text-[10px] text-slate-400 block uppercase mb-1">Incident Commander</span>
      <span class="font-bold text-slate-800">Tier 3 Platform SRE (Beth Anglin)</span>
    </div>
  </div>

  <!-- 1-Click Mitigation Playbooks -->
  <div>
    <span class="text-xs font-mono font-bold text-slate-700 block uppercase mb-2.5">
      1-Click Incident Mitigation Playbooks:
    </span>
    
    <div class="space-y-2">
      {#each playbooks as pb}
        <div class="flex items-center justify-between gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 hover:border-slate-300 transition-all">
          <div class="flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-slate-400"></span>
            <div>
              <span class="text-xs font-mono font-bold text-slate-900 block">{pb.title}</span>
              <span class="text-[10px] font-mono text-slate-500">{pb.action} {pb.automated ? '• Automated Execution' : ''}</span>
            </div>
          </div>

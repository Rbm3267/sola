...
>
  ...
  <p class="text-[11px] text-slate-400 mt-0.5">Triggers Sola Context Menu with keyboard shortcuts</p>
</div>
    ...
    {#each items as item}
      ...
      {:else}
        ...
        >
          ...
          {#if item.shortcut}
            <span class="text-[10px] font-mono opacity-50">{item.shortcut}</span>
          {/if}
        </button>
      {/if}
    {/each}

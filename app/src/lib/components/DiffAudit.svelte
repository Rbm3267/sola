<script lang="ts">
  ...
  function getRiskBadge(risk: string = 'Moderate') {
    switch (risk.toLowerCase()) {
      ...
      case 'destructive': return 'bg-rose-50 text-rose-900 border-rose-200';
...
<div class="relative bg-white/95 backdrop-blur-2xl border border-slate-200/90 rounded-3xl p-6 sm:p-7 shadow-sm hover:shadow-md transition-all overflow-hidden">
  ...
  <!-- Metadata Sub-bar -->
  <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-5 text-xs font-mono text-slate-600 bg-slate-50/80 p-3 rounded-2xl border border-slate-100">
    <div>
      <span class="text-[10px] text-slate-400 block uppercase">Requester</span>
      <span class="font-bold text-slate-800">{config.requester || 'Platform SRE Team'}</span>
    </div>
    <div>
      <span class="text-[10px] text-slate-400 block uppercase">Release Window</span>
      <span class="font-bold text-slate-800">{config.window || 'Tonight 02:00 UTC'}</span>
    </div>
  ...
  <!-- Syntax Diff Canvas -->
  <div class="bg-slate-950 text-white rounded-2xl p-4 font-mono text-xs overflow-x-auto mb-6 border border-slate-800">
  ...
  <div class="flex items-center justify-between gap-3 pt-2">
    ...
    <div class="flex items-center gap-2">
      ...
      {:else}
        <div class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-rose-50 text-rose-800 text-xs font-mono font-bold border border-rose-200">

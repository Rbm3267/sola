...
{#if isOpen}
  ...
  <div 
    ...
    class="fixed inset-0 z-50 bg-slate-950 dark:bg-white/60 backdrop-blur-md flex items-center justify-center p-3 sm:p-4">
    ...
    <div 
      ...
      class="bg-white dark:bg-white/[0.02] border border-slate-200/90 dark:border-white/[0.04] rounded-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] max-w-2xl w-full flex flex-col overflow-hidden max-h-[85vh] relative">
      
      <!-- Top Title Header -->
      <div class="p-4 sm:p-5 border-b border-slate-100 dark:border-white/[0.04] flex items-center justify-between bg-slate-50/60 dark:bg-white/[0.04]">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              <h2 id="assistant-title" class="text-sm font-black text-slate-950 dark:text-slate-50 tracking-tight">Sola Arc</h2>
              <span class="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-500/20 text-emerald-900 font-mono text-[9px] font-bold">Solutions AI</span>
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400 font-normal">Autonomous UI & Systems Solutions Intelligence</p>
          </div>
        </div>
        ...
        <button 
          ...
          class="w-7 h-7 rounded-full bg-slate-200/60 hover:bg-slate-200 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white flex items-center justify-center transition-all cursor-pointer">
          <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
      </div>

      <!-- Scrollable Message Conversation Stream -->
      <div class="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4 font-sans text-xs sm:text-sm bg-white dark:bg-white/[0.02] min-h-[260px]">
        {#each messages as msg, idx}
          <div class="flex flex-col gap-1.5 {msg.role === 'user' ? 'items-end' : 'items-start'}">
            ...
            <div class="p-4 rounded-2xl max-w-[92%] leading-relaxed {msg.role === 'user' ? 'bg-slate-900 text-white rounded-br-xs shadow-xs' : 'bg-slate-50 dark:bg-white/[0.04] border border-slate-200/90 dark:border-white/[0.04] text-slate-800 dark:text-slate-200 rounded-bl-xs'}">
            ...
            {#if msg.role === 'assistant'}
              <button 
                ...
                class="px-2 py-1 text-[10px] font-mono text-slate-400 hover:text-slate-700 dark:text-slate-300 flex items-center gap-1 transition-colors cursor-pointer">
                {#if copiedIndex === idx}
      ...
      <div class="px-4 sm:px-6 py-2.5 bg-slate-50/80 dark:bg-white/[0.04] border-t border-slate-100 dark:border-white/[0.04] flex items-center gap-1.5 overflow-x-auto no-scrollbar">
        ...
        {#each quickPrompts as qp}
          <button 
            ...
            class="px-2.5 py-1 rounded-full bg-white dark:bg-white/[0.02] hover:bg-slate-100 dark:bg-white/[0.08] border border-slate-200/80 dark:border-white/[0.04] text-slate-700 dark:text-slate-300 font-medium text-xs whitespace-nowrap transition-all shrink-0 cursor-pointer shadow-2xs">
            {qp}
          </button>
        {/each}
      </div>
      ...
      <form 
        ...
        class="p-3 sm:p-4 bg-white dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/[0.04] flex items-center gap-2">
        <input 
          type="text" 
          bind:value={prompt}
          placeholder="Ask Sola Arc (e.g. How do I embed this into my React app?)..." 
          class="flex-1 bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.04] rounded-2xl px-4 py-2.5 text-xs sm:text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-400 font-sans transition-all" />
        <button 
          type="submit"
